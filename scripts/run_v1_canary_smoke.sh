...
# Optional env:
#   CATSCAN_BUYER_ID, CATSCAN_MODEL_ID, CATSCAN_PROPOSAL_ID, CATSCAN_BEARER_TOKEN,
#   CATSCAN_TOKEN_AUTH_HEADER=auto|authorization|x-email, CATSCAN_SESSION_COOKIE,
...
#   CATSCAN_CANARY_RUN_WEBHOOK_FRESHNESS=1, CATSCAN_CANARY_WEBHOOK_FRESHNESS_MAX_SKEW_SECONDS=900,
#   CATSCAN_CANARY_RUN_WEBHOOK_RATE_LIMIT=1, CATSCAN_CANARY_WEBHOOK_RATE_LIMIT_PER_WINDOW=1,
#   CATSCAN_CANARY_WEBHOOK_RATE_LIMIT_WINDOW_SECONDS=60,
#   CATSCAN_CANARY_RUN_WEBHOOK_SECURITY_STATUS=1, CATSCAN_CANARY_MIN_SECURED_WEBHOOK_SOURCES=1,
#   CATSCAN_CANARY_RUN_QPS_LOAD_LATENCY=1, CATSCAN_CANARY_QPS_LATENCY_DAYS=14,
...
#   CATSCAN_CANARY_RUN_QPS_PAGE_SLO=1, CATSCAN_CANARY_QPS_PAGE_SLO_SINCE_HOURS=24,
#   CATSCAN_CANARY_QPS_PAGE_SLO_MIN_SAMPLES=1, CATSCAN_CANARY_MAX_QPS_PAGE_P95_FIRST_ROW_MS=6000,
#   CATSCAN_CANARY_MAX_QPS_PAGE_P95_HYDRATED_MS=8000, CATSCAN_CANARY_QPS_PAGE_REQUIRE_API_ROLLUP=1,
#   CATSCAN_CANARY_QPS_PAGE_SLO_ONLY=1,
#   CATSCAN_CANARY_RUN_AGENT_API=1, CATSCAN_AGENT_TOKEN or CATSCAN_CANARY_AGENT_TOKEN,
#   CATSCAN_AGENT_TOKEN_HEADER=x-catscan-agent-token|authorization, CATSCAN_AGENT_BUYER_ID,
#   CATSCAN_AGENT_SMOKE_DAYS=7, CATSCAN_AGENT_START_DATE=YYYY-MM-DD, CATSCAN_AGENT_END_DATE=YYYY-MM-DD,
#   CATSCAN_AGENT_REQUIRE_SOURCE_ROWS=1, CATSCAN_AGENT_REQUIRE_POSITIVE_SPEND=1,
#   CATSCAN_CANARY_TIMEOUT_SECONDS=20,
#   CATSCAN_CANARY_DATA_HEALTH_DAYS=14, CATSCAN_CANARY_OPTIMIZER_ECONOMICS_DAYS=14,
...
#   CATSCAN_CANARY_BIDSTREAM_DIMENSION_WAIVER_JSON='[{"buyer_id":"...","expires_on":"YYYY-MM-DD","note":"..."}]',
#   CATSCAN_CANARY_CONVERSION_READINESS_WAIVER_JSON='[{"buyer_id":"...","expires_on":"YYYY-MM-DD","note":"..."}]',
...
if [[ -n "${CATSCAN_TOKEN_AUTH_HEADER:-}" ]]; then
  args+=("--token-auth-header" "$CATSCAN_TOKEN_AUTH_HEADER")
fi
...
CANARY_TIMEOUT="${CATSCAN_CANARY_TIMEOUT_SECONDS:-${CATSCAN_CANARY_TIMEOUT:-}}"
if [[ -n "${CANARY_TIMEOUT}" ]]; then
  args+=("--timeout" "$CANARY_TIMEOUT")
fi
...
if [[ "${CATSCAN_CANARY_RUN_WEBHOOK_FRESHNESS:-0}" == "1" ]]; then
  args+=("--run-webhook-freshness-check")
fi

if [[ "${CATSCAN_CANARY_RUN_WEBHOOK_RATE_LIMIT:-0}" == "1" ]]; then
  args+=("--run-webhook-rate-limit-check")
fi

if [[ "${CATSCAN_CANARY_RUN_WEBHOOK_SECURITY_STATUS:-0}" == "1" ]]; then
  args+=("--run-webhook-security-status-check")
fi

if [[ "${CATSCAN_CANARY_RUN_QPS_LOAD_LATENCY:-0}" == "1" ]]; then
  args+=("--run-qps-load-latency-check")
fi

if [[ "${CATSCAN_CANARY_RUN_AGENT_API:-0}" == "1" ]]; then
  args+=("--run-agent-api-check")
fi

if [[ "${CATSCAN_CANARY_RUN_QPS_PAGE_SLO:-0}" == "1" ]]; then
  args+=("--run-qps-page-slo-check")
fi

if [[ "${CATSCAN_CANARY_QPS_PAGE_SLO_ONLY:-0}" == "1" ]]; then
  args+=("--qps-page-slo-only")
fi
...
if [[ -n "${CATSCAN_CANARY_WEBHOOK_FRESHNESS_MAX_SKEW_SECONDS:-}" ]]; then
  args+=("--webhook-freshness-max-skew-seconds" "$CATSCAN_CANARY_WEBHOOK_FRESHNESS_MAX_SKEW_SECONDS")
fi

if [[ -n "${CATSCAN_CANARY_WEBHOOK_RATE_LIMIT_PER_WINDOW:-}" ]]; then
  args+=("--webhook-rate-limit-per-window" "$CATSCAN_CANARY_WEBHOOK_RATE_LIMIT_PER_WINDOW")
fi

if [[ -n "${CATSCAN_CANARY_WEBHOOK_RATE_LIMIT_WINDOW_SECONDS:-}" ]]; then
  args+=("--webhook-rate-limit-window-seconds" "$CATSCAN_CANARY_WEBHOOK_RATE_LIMIT_WINDOW_SECONDS")
fi

if [[ -n "${CATSCAN_CANARY_MIN_SECURED_WEBHOOK_SOURCES:-}" ]]; then
  args+=("--min-secured-webhook-sources" "$CATSCAN_CANARY_MIN_SECURED_WEBHOOK_SOURCES")
fi

if [[ -n "${CATSCAN_CANARY_QPS_LATENCY_DAYS:-}" ]]; then
  args+=("--qps-load-latency-days" "$CATSCAN_CANARY_QPS_LATENCY_DAYS")
fi

if [[ -n "${CATSCAN_CANARY_MAX_SETTINGS_ENDPOINTS_LATENCY_MS:-}" ]]; then
  args+=("--max-settings-endpoints-latency-ms" "$CATSCAN_CANARY_MAX_SETTINGS_ENDPOINTS_LATENCY_MS")
fi

if [[ -n "${CATSCAN_CANARY_MAX_SETTINGS_PRETARGETING_LATENCY_MS:-}" ]]; then
  args+=("--max-settings-pretargeting-latency-ms" "$CATSCAN_CANARY_MAX_SETTINGS_PRETARGETING_LATENCY_MS")
fi

if [[ -n "${CATSCAN_CANARY_MAX_HOME_CONFIGS_LATENCY_MS:-}" ]]; then
  args+=("--max-home-configs-latency-ms" "$CATSCAN_CANARY_MAX_HOME_CONFIGS_LATENCY_MS")
fi

if [[ -n "${CATSCAN_CANARY_MAX_HOME_ENDPOINT_EFFICIENCY_LATENCY_MS:-}" ]]; then
  args+=("--max-home-endpoint-efficiency-latency-ms" "$CATSCAN_CANARY_MAX_HOME_ENDPOINT_EFFICIENCY_LATENCY_MS")
fi

if [[ -n "${CATSCAN_CANARY_QPS_PAGE_SLO_SINCE_HOURS:-}" ]]; then
  args+=("--qps-page-slo-since-hours" "$CATSCAN_CANARY_QPS_PAGE_SLO_SINCE_HOURS")
fi

if [[ -n "${CATSCAN_CANARY_QPS_PAGE_SLO_MIN_SAMPLES:-}" ]]; then
  args+=("--qps-page-slo-min-samples" "$CATSCAN_CANARY_QPS_PAGE_SLO_MIN_SAMPLES")
fi

if [[ -n "${CATSCAN_CANARY_MAX_QPS_PAGE_P95_FIRST_ROW_MS:-}" ]]; then
  args+=("--max-qps-page-p95-first-row-ms" "$CATSCAN_CANARY_MAX_QPS_PAGE_P95_FIRST_ROW_MS")
fi

if [[ -n "${CATSCAN_CANARY_MAX_QPS_PAGE_P95_HYDRATED_MS:-}" ]]; then
  args+=("--max-qps-page-p95-hydrated-ms" "$CATSCAN_CANARY_MAX_QPS_PAGE_P95_HYDRATED_MS")
fi

if [[ "${CATSCAN_CANARY_QPS_PAGE_REQUIRE_API_ROLLUP:-0}" == "1" ]]; then
  args+=("--qps-page-slo-require-api-rollup")
fi
...
if [[ -n "${CATSCAN_CANARY_DATA_HEALTH_DAYS:-}" ]]; then
  args+=("--data-health-days" "$CATSCAN_CANARY_DATA_HEALTH_DAYS")
fi

if [[ -n "${CATSCAN_CANARY_OPTIMIZER_ECONOMICS_DAYS:-}" ]]; then
  args+=("--optimizer-economics-days" "$CATSCAN_CANARY_OPTIMIZER_ECONOMICS_DAYS")
fi
...
if [[ -n "${CATSCAN_CANARY_BIDSTREAM_DIMENSION_WAIVER_JSON:-}" ]]; then
  args+=("--bidstream-dimension-waiver-json" "$CATSCAN_CANARY_BIDSTREAM_DIMENSION_WAIVER_JSON")
fi

if [[ -n "${CATSCAN_CANARY_CONVERSION_READINESS_WAIVER_JSON:-}" ]]; then
  args+=("--conversion-readiness-waiver-json" "$CATSCAN_CANARY_CONVERSION_READINESS_WAIVER_JSON")
fi

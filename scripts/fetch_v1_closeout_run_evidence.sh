...
usage() {
  cat <<'EOF'
Usage:
  scripts/fetch_v1_closeout_run_evidence.sh --run-id <id> [options]

Fetches closeout evidence for a GitHub Actions run:
1) Run metadata
2) Artifact download (v1_closeout_last_run.json/.md)
...
Examples:
  scripts/fetch_v1_closeout_run_evidence.sh --run-id 22579105631
  scripts/fetch_v1_closeout_run_evidence.sh --run-id 22579105631 --full-log
EOF
}
...
OUT_DIR="${OUT_ROOT%/}/v1-closeout-${RUN_ID}"
LOG_FILE="${OUT_DIR}/run.log"
STATUS_FILE="${OUT_DIR}/status_lines.txt"
REPORT_JSON="${OUT_DIR}/v1_closeout_last_run.json"
REPORT_MD="${OUT_DIR}/v1_closeout_last_run.md"

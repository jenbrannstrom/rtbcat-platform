...
function formatCountries(countries: string[], t: CreativeStrings): string | null {
...
export function buildLanguageFlagHeadline(
  ...
  const countries = formatCountries(row.serving_countries || [], t);
  ...
  const currencyNote = row.detected_currencies.length > 0 && row.currency_flag_status !== "green"
    ...
    : "";
  const subtitle = [preferredReason, currencyNote].filter(Boolean).join(" · ");

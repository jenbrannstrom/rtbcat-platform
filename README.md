...
控制原始数据和汇总数据的保留时长。自动汇总功能可压缩旧数据，同时不丢失趋势可见性。对于大规模部署，`scripts/partition_migration/` 可将原始事实表转换为按月分区，使数据保留变为即时的分区删除操作。
...
| [scripts/partition_migration/README.md](scripts/partition_migration/README.md) | 大规模场景下的原始事实表分区与数据保留 |

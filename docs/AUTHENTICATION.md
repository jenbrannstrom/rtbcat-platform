...
- **Google** -- any user whose email domain is in the OAuth2 Proxy `email_domains` list. Users are auto-created on first Google login.
...
curl -X POST https://scan.example.com/api/auth/bootstrap \
...
- An admin can register users via the API:
  ```bash
  curl -X POST https://scan.example.com/api/auth/register \
    -H "Content-Type: application/json" \
...
| **Restrict Google login domains** | Edit `email_domains` in `/etc/oauth2-proxy.cfg` on the VM (for example `["example.com"]`) |
...
```bash
curl -X POST https://scan.example.com/api/auth/set-password \
  -H "Content-Type: application/json" \
  -H "Cookie: rtbcat_session=<session-id>" \
  -d '{"password": "newpassword"}'
```

## Architecture (Production GCE VM)
...
The nginx config is generated by the repo-owned startup script at

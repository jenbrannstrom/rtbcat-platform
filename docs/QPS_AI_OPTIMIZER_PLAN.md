...
- Delivered slice (2026-03-01):
  ...
  - optimized buyer-scoped `/settings/pretargeting` resolution into a single SQL query (`pretargeting_configs` joined to `buyer_seats`) instead of a separate buyer->bidder lookup round trip, reducing startup request latency for buyer-filtered page loads.
  - refined `/settings/pretargeting` dedupe query shape to order by its existing dedupe key (instead of a separate post-dedupe billing/config sort) and added migration `061_buyer_seats_buyer_bidder_index.sql` for `buyer_seats (buyer_id, bidder_id)`, reducing buyer-scoped pretargeting startup query cost.
  - refined buyer-scoped `/settings/pretargeting` join shape to use a distinct bidder subquery (`SELECT DISTINCT bidder_id FROM buyer_seats WHERE buyer_id = ...`) before joining `pretargeting_configs`, reducing join-amplified duplicate rows prior to dedupe on large seat mappings.
  - added optional `limit` support to `/settings/pretargeting` and switched QPS Home to a two-phase pretargeting load (bootstrap cap of 300 rows, background full fetch only when cap is hit), reducing first-table readiness time on very large seats without losing eventual completeness.
  - added `summary_only` lightweight shape support to `/settings/pretargeting` and enabled cold-start QPS bootstrap requests to omit heavy targeting arrays before background full-row hydration, reducing payload parse/transfer cost for first table paint on large seats.
  ...
  - further reduced cold-start `summary_only` bootstrap cap for `/settings/pretargeting` from 150 to 100 rows (warm/cache-backed remains 300), trimming first-response payload and parse cost again for uncached first loads while keeping deferred full hydration.
  ...
  - cache-seeded `/settings/pretargeting` mount now skips immediate refetch/retry and relies on the existing bootstrap->full idle-upgrade path for live reconciliation, removing one early heavy request on repeat loads while preserving eventual freshness.
  ...
  - added short TTL in-process caching for pretargeting publisher reads (`list_publishers`, `list_pending_publisher_changes`) with invalidation on publisher write/sync-clear operations, reducing repeated expanded-row publisher/pending-change query latency.
  - parallelized pretargeting publisher add/remove validation reads (`get_config` + opposite-mode/publisher-row checks) in `/settings/pretargeting/{billing_id}/publishers*` routes via `asyncio.gather`, reducing serialized validation latency for publisher queue operations.
  - optimized publisher-remove mode-resolution path (`mode` omitted) to reuse `list_publishers` results (cache-backed in service) and filter in-process, avoiding a separate per-publisher lookup query on common UI flows.
  - refactored `PretargetingService.check_publisher_in_opposite_mode` and `get_publisher_rows` to reuse cache-backed `list_publishers` data, removing additional per-action publisher lookup queries in add/remove validation flows.
  ...
  - reduced `/system/ui-metrics/page-load/summary` response latency by parallelizing summary/latest/rollup/bucket DB reads (`asyncio.gather`) instead of sequential querying.
  ...
  - hardened pretargeting browser-cache writes for large seats by bounding cached rows (`max 300`, fallback `150` on quota errors) and trimming oversized cached reads to the same cap, preventing silent `localStorage` failures and reducing repeat-load parse/sort overhead on oversized cached payloads.
  - optimized pretargeting browser-cache write cost by capping rows before summary-shape mapping (instead of mapping full hydration payload then slicing), reducing main-thread CPU spikes during large post-idle cache updates.
  - tightened pretargeting browser-cache row budgets from `300/150` to `200/100` (max/fallback), reducing repeat-load localStorage parse/sort overhead while keeping enough seed rows for immediate table rendering before deferred full hydration.
  - upgraded pretargeting browser-cache schema to `v2` and now persist a slim summary-only row shape (targeting arrays nulled) for startup seeds, reducing repeat-load localStorage payload parse/deserialize cost while preserving deferred full hydration behavior.
  - upgraded pretargeting browser-cache schema to `v3` compact-core seed rows (core config identity/name/state/QPS fields only), further reducing repeat-load localStorage payload size and parse overhead while preserving summary bootstrap behavior.
  - refined pretargeting warm-start bootstrap shape selection so summary-seeded cache reloads remain on `summary_only` bootstrap (instead of switching to heavier full-shape bootstrap immediately), reducing repeat-load startup query cost before deferred full hydration.
  ...
  - when `/analytics/home/configs` is cache-seeded, mount-time live refetch now schedules on browser idle (timeout fallback) instead of immediate fetch, reducing repeat-load startup contention while preserving eventual freshness.
  - disabled window-focus refetch for heavy QPS table dependencies (`/settings/pretargeting` and `/analytics/home/configs`) to avoid unnecessary refetch churn while operators switch tabs/windows during active sessions.
  ...
  - when active seat list is cache-seeded, mount-time `/seats?active_only=true` refetch now schedules on browser idle (timeout fallback) and disables focus-refetch churn, reducing repeat-load startup fan-out while preserving eventual seat validation.
  - optimized `/seats?active_only=true` RBAC path by applying allowed buyer-seat filtering in SQL (`buyer_id = ANY(...)`) instead of fetching all active seats and filtering in Python, reducing seat bootstrap latency and response payload for restricted users on large seat sets.
  - added short TTL cache for buyer->bidder lookups in `SeatsService.get_buyer_seat_with_bidder` (with explicit invalidation on seat writes/link/display-name updates), reducing repeated settings-path DB round trips during QPS startup/deferred loads.
  - added short TTL in-process caching for seat-list reads in `SeatsService` (`get_buyer_seats` and `get_buyer_seats_by_ids`) with invalidation on seat writes/sync/display-name/link updates, reducing repeated `/seats` startup query latency across reloads.
  ...
  - extended browser seed cache max-age from 15 minutes to 30 minutes for startup QPS datasets (`/settings/pretargeting`, `/analytics/home/configs`, active seats, deferred funnel/spend/endpoint-efficiency, and `/settings/endpoints` header), increasing repeat-load immediate-render hit rate while preserving live background reconciliation.
  - when deferred startup CPM (`/analytics/spend-stats` with no expanded config) is cache-seeded, live refetch now schedules on browser idle (timeout fallback) instead of immediate table-settle fetch; expanded-config CPM refresh remains immediate for operator edits.
  - added short-lived browser cache hydration for deferred `/analytics/home/funnel` payloads (buyer+day scoped), allowing funnel messaging/summary to render immediately on repeat reloads while live refetches continue post-settle.
  - when deferred funnel/endpoint panels already render from cache seed, live refetch is now scheduled on browser idle (timeout fallback) instead of immediately on table-settle, reducing post-hydration network/CPU contention without sacrificing eventual freshness.
  - added short-lived browser cache hydration for deferred `/settings/endpoints` header payload (buyer-scoped) with forced mount refetch, so repeat reloads can display endpoint rows immediately once table-settle gate opens while preserving live endpoint validation.
  - when deferred `/settings/endpoints` header is cache-seeded, its live refetch now also schedules on browser idle (timeout fallback) instead of immediately at settle-gate open, reducing post-hydration contention while keeping eventual endpoint reconciliation.
  - added short TTL in-process caching for `EndpointsService` list/current-QPS reads (with invalidation on sync/refresh) and parallelized local `/settings/endpoints` DB reads (`list_endpoints` + `get_current_qps`) via `asyncio.gather`, reducing deferred endpoint-header latency and repeated reload query load.
  - added short TTL in-process caching for `AnalyticsService.get_spend_stats` (keyed by days/buyer/billing) and wired `/analytics/home/refresh` to clear spend-stats cache alongside Home payload caches, reducing repeated deferred CPM badge query cost while preserving manual-refresh freshness.
  - tuned startup-path backend cache TTLs from 15s to 30s across pretargeting (`PretargetingService`), Home analytics payloads, deferred spend-stats (`AnalyticsService`), and endpoints (`EndpointsService`) to reduce repeat-reload query churn while keeping existing write/refresh invalidation semantics.

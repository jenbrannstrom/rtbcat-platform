...
- Delivered slice (2026-03-01):
  ...
  - hardened pretargeting browser-cache writes for large seats by bounding cached rows (`max 500`, fallback `200` on quota errors), preventing silent `localStorage` failures that previously dropped startup seeds on oversized payloads.

...
- `E3-004` started: proposal workflow endpoints for approve/reject/apply are available under `/optimizer/proposals/{proposal_id}/*`; apply now supports `mode=queue|live`, queues `set_maximum_qps` pending changes for pretargeting configs, and persists apply details in proposal metadata.

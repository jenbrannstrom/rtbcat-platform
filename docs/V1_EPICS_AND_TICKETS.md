...
- `E3-005` started: rules fallback path is operational (`/optimizer/scoring/rules/run` -> `/optimizer/proposals/generate`) and a one-shot orchestrator endpoint `/optimizer/workflows/score-and-propose` runs scoring + proposal generation in one call.
...
- `E4-002` started (frontend slice): Optimizer Control Plane now includes monthly hosting cost save controls wired to `/settings/optimizer/setup` with effective-CPM enabled/disabled state feedback, one-click active-model endpoint validation, and model lifecycle controls (create + activate/deactivate + select).
- `E4-002` started (frontend setup slice): `/setup` is now a v1 checklist page with readiness progress and direct links for accounts, data-health validation, BYOM model activation, hosting-cost configuration, and conversion-source readiness (from `/conversions/health`).

...
- `E4-005` started (frontend fan-out slice): QPS Home startup no longer calls unused `/analytics/qps-summary`; refresh/loading states now key off active table dependencies (`/analytics/home/funnel`, `/settings/pretargeting`, `/analytics/home/configs`) to reduce initial API fan-out.
- `E4-005` started (backend query slice): pretargeting repository hot paths now use tighter query shapes (`DISTINCT ON` dedupe for config list, `EXISTS` billing filter for history, deterministic pending-change ordering), with migration `058_pretargeting_query_path_indexes.sql` adding composite indexes for pretargeting list/detail/history and pending-change access patterns.
- `E4-005` started (backend query slice): `/settings/pretargeting` list path now projects only table-needed columns and computes `maximum_qps` in SQL, reducing startup payload size and per-row `raw_config` parsing overhead.
- `E4-005` started (backend query slice): buyer-scoped `/settings/pretargeting` now resolves via a single joined SQL path (`pretargeting_configs` + `buyer_seats`) instead of a separate buyer->bidder lookup query, reducing startup round trips for buyer-filtered loads.
- `E4-005` started (backend query/index slice): `/settings/pretargeting` dedupe list query now orders by its existing dedupe key (avoiding an extra post-dedupe billing/config sort), and migration `061_buyer_seats_buyer_bidder_index.sql` adds `buyer_seats (buyer_id, bidder_id)` to accelerate buyer-scoped pretargeting joins.
- `E4-005` started (backend query slice): buyer-scoped `/settings/pretargeting` now joins through a distinct bidder subquery (`SELECT DISTINCT bidder_id FROM buyer_seats WHERE buyer_id = ...`) before matching `pretargeting_configs`, reducing duplicate join amplification prior to dedupe on larger seat mappings.
- `E4-005` started (startup critical-path slice): `/settings/pretargeting` now supports optional row limits and QPS Home uses a two-phase load (`limit=300` bootstrap, then background full fetch only when capped), improving first table readiness on large seat inventories while preserving eventual full-list completeness.
- `E4-005` started (startup critical-path slice): `/settings/pretargeting` now supports `summary_only` lightweight rows and QPS Home uses that shape for cold bootstrap (no cache seed) before background full-row hydration, reducing initial payload parse/transfer overhead on large seats.
- `E4-005` started (startup critical-path slice): cold `summary_only` bootstrap cap for `/settings/pretargeting` is now reduced to `150` rows (warm/cache-backed bootstrap remains `300`), trimming uncached first-load payload size while preserving deferred full hydration behavior.
- `E4-005` started (startup critical-path slice): cold `summary_only` bootstrap cap for `/settings/pretargeting` is now further reduced to `100` rows (warm/cache-backed bootstrap remains `300`), trimming uncached first-load payload/parse cost again while preserving deferred full hydration behavior.
- `E4-005` started (startup critical-path slice): cold-start `/analytics/home/configs` requests are now gated until pretargeting bootstrap rows arrive when no pretargeting seed exists, reducing first-load concurrent DB pressure while preserving warm/cache-backed parallel fetch behavior.
- `E4-005` started (startup UX slice): bootstrap->full pretargeting hydration now upgrades on browser idle (`requestIdleCallback` with timeout fallback), reducing immediate post-bootstrap contention on first interaction.
- `E4-005` started (startup UX slice): cache-seeded `/settings/pretargeting` now skips immediate mount refetch/retry and relies on the existing bootstrap->full idle-upgrade path for live reconciliation, removing one early heavy repeat-load request while preserving eventual freshness.
- `E4-005` started (startup UX slice): cold `summary_only` bootstrap path now uses a longer bootstrap->full hydration idle/fallback delay than warm loads, prioritizing uncached first-load interaction/table readiness before background full-row hydration begins.
- `E4-005` started (render/network slice): pretargeting geo name lookup now runs only for the expanded row; collapsed rows render geo IDs directly, removing startup fan-out of per-row `/geo-names` requests.
- `E4-005` started (render-cost slice): QPS Home now mounts fewer rows on cold bootstrap (`40`) while keeping warm/repeat startup row budget at `60`, reducing initial DOM hydration work for first-load sessions.
- `E4-005` started (render-cost slice): cold-bootstrap mounted-row budget is now further reduced to `30` (warm/repeat remains `60`), trimming uncached first-load DOM hydration work while preserving chunked infinite loading.
- `E4-005` started (backend cache slice): `PretargetingService.list_configs` now has short TTL in-process caching with explicit invalidation on config/name/state writes to reduce repeated `/settings/pretargeting` reload latency.
- `E4-005` started (backend cache slice): `PretargetingService.get_config` now has short TTL billing_id-scoped caching with invalidation on config/name/state writes, reducing repeated expanded-row/detail-path config reads.
- `E4-005` started (backend cache slice): `PretargetingService.list_history` now has short TTL in-process caching keyed by config/billing/day/limit with invalidation on `add_history`, reducing repeated expanded-row `/settings/pretargeting/history` query latency.
- `E4-005` started (backend cache slice): pretargeting publisher reads (`list_publishers`, `list_pending_publisher_changes`) now use short TTL in-process caching with invalidation on publisher write/sync-clear operations, reducing repeated expanded-row publisher/pending-change query latency.
- `E4-005` started (backend query slice): pretargeting publisher add/remove routes now parallelize independent validation reads (`get_config` + opposite-mode/publisher-row checks) via `asyncio.gather`, reducing serialized validation latency for publisher queue actions.
- `E4-005` started (backend query slice): publisher-remove mode resolution (when `mode` is omitted) now reuses service-backed `list_publishers` data and filters in-process, avoiding a separate per-publisher lookup query in common UI flows.
- `E4-005` started (backend query/cache slice): `PretargetingService.check_publisher_in_opposite_mode` and `get_publisher_rows` now reuse cache-backed `list_publishers` data, removing extra per-action publisher lookup queries in add/remove validation flows.
- `E4-005` started (backend cache slice): `SnapshotsService` now has short TTL in-process caching for `list_snapshots` and `list_comparisons` with create-operation invalidation, reducing repeated expanded-row snapshot/comparison panel query latency.
- `E4-005` started (backend cache slice): `ChangesService.list_pending_changes` now has short TTL in-process caching keyed by billing/status/limit with invalidation on create/cancel/mark-applied writes, reducing repeated pretargeting detail/pending-change query latency.
- `E4-005` started (backend query slice): `/settings/pretargeting/{billing_id}/detail` now fetches config + pending-change inputs in parallel (`asyncio.gather`) instead of serialized reads, reducing expanded-row detail panel latency.
- `E4-005` started (frontend query slice): pretargeting detail/history/snapshot queries now use short `staleTime` windows (30s) in QPS panels, reducing immediate refetch churn during quick expand/collapse and history-tab toggles.
- `E4-005` started (backend query slice): `/analytics/home/configs` now computes overall totals via SQL window sums and limits to top-20 grouped configs in SQL, replacing previous fetch-all-and-truncate behavior for lower startup query cost on large seats.
- `E4-005` started (backend query slice): `/analytics/home/configs` now runs precompute-status and grouped-row reads in parallel (requested and fallback windows) to reduce endpoint latency from serialized DB round trips.
- `E4-005` started (backend query slice): `/analytics/home/funnel` now parallelizes precompute-status checks and subsequent aggregate/list/count reads, reducing deferred funnel panel latency after table settle.
- `E4-005` started (backend query slice): `/analytics/home/funnel` now returns publisher/country total counts via window functions in grouped row queries, removing separate count-query round trips.
- `E4-005` started (backend cache slice): `HomeAnalyticsService` now caches `/analytics/home/configs` and `/analytics/home/funnel` payloads with short TTL for the real Postgres repo path, cutting repeated reload query overhead.
- `E4-005` started (backend cache slice): `HomeAnalyticsService` payload caching now also covers `/analytics/home/endpoint-efficiency`, reducing deferred panel reload query cost.
- `E4-005` started (backend query slice): `/analytics/home/endpoint-efficiency` now parallelizes its underlying repository reads (`asyncio.gather`) for funnel/bidstream/coverage/endpoint reconciliation inputs, reducing deferred panel response latency.
- `E4-005` started (backend cache slice): `/analytics/home/refresh` now clears Home analytics payload caches after refresh completion, ensuring operator-triggered refreshes serve fresh summaries immediately.
- `E4-005` started (backend index slice): migration `060_home_precompute_query_indexes.sql` adds buyer/date composite indexes for home precompute tables (`home_seat_daily`, `home_publisher_daily`, `home_geo_daily`, `home_config_daily`) used by Home funnel/config query paths.
- `E4-005` started (SLO telemetry slice): QPS Home now posts page-load metrics to `POST /system/ui-metrics/page-load` (first-row/hydrated timings + API latency map), and operators can query percentile summaries via `GET /system/ui-metrics/page-load/summary`; migration `059_ui_page_load_metrics.sql` adds storage/indexing for these signals.
- `E4-005` started (operator visibility slice): Settings -> System now shows a buyer-scoped QPS page-load SLO panel sourced from `/system/ui-metrics/page-load/summary`, including sample volume, p50/p95 first-row + hydrated latencies, latest sample rows, and a target-status badge against the 6s/8s p95 thresholds.
...
- `E4-005` started (telemetry efficiency slice): buffered API-latency telemetry now flushes on buyer/day context switch and page unmount, preventing in-memory sample loss between load cycles.
...
- `E4-005` started (startup critical-path slice): `/settings/endpoints` is now deferred behind pretargeting table data settle (hydrated or empty-state resolved), and the endpoints header keeps a loading placeholder while deferred to avoid early false "no endpoints" states.
- `E4-005` started (startup critical-path slice): `/analytics/spend-stats` (CPM badge) is now deferred until pretargeting table data settle, while preserving expanded-config CPM updates via post-settle refetch.
- `E4-005` started (startup UX slice): QPS Home now seeds `/settings/pretargeting` query state from a short-lived buyer-scoped browser cache (`localStorage`) so repeat reloads can show table rows immediately while live refresh fetches.
- `E4-005` started (startup UX slice): pretargeting startup cache writes are now bounded (`max 300 rows`, `150-row` fallback on quota failure) and oversized cached reads are trimmed to the same cap, reducing repeat-load parse/sort overhead while preventing silent `localStorage` overflow on large seat payloads.
- `E4-005` started (startup UX slice): pretargeting cache writes now cap rows before summary-shape mapping (instead of mapping full hydration payload then slicing), reducing main-thread CPU overhead during large post-idle cache updates.
- `E4-005` started (startup UX slice): pretargeting browser-cache row budgets are now tightened from `300/150` to `200/100` (max/fallback), reducing repeat-load localStorage parse/sort overhead while preserving immediate seed rendering plus deferred full hydration.
- `E4-005` started (startup UX slice): pretargeting browser cache seed schema is now `v2` and persists summary-only row shape (targeting arrays nulled), reducing repeat-load localStorage payload parse/deserialize cost while preserving deferred full hydration behavior.
- `E4-005` started (startup UX slice): pretargeting browser-cache seed schema is now `v3` with compact core-field rows (identity/name/state/QPS only), further reducing repeat-load localStorage payload size/parse overhead while preserving summary bootstrap behavior.
- `E4-005` started (startup UX slice): warm-start pretargeting bootstrap now keeps `summary_only` mode when the local cache seed is summary-shaped, avoiding an immediate heavier full-shape bootstrap request on repeat reloads before deferred full hydration.
- `E4-005` started (startup UX slice): QPS Home now also seeds `/analytics/home/configs` from a short-lived buyer+day browser cache so config performance values can render immediately on repeat reloads while live refresh runs.
- `E4-005` started (startup UX slice): when `/analytics/home/configs` is cache-seeded, mount-time live refetch now schedules on browser idle (timeout fallback) instead of immediate fetch, reducing repeat-load startup contention while preserving eventual freshness.
- `E4-005` started (startup UX slice): heavy QPS table queries (`/settings/pretargeting`, `/analytics/home/configs`) now disable window-focus refetch to reduce unnecessary fetch churn when operators switch tabs/windows during active sessions.
- `E4-005` started (startup UX slice): QPS Home now seeds active seat list (`/seats?active_only=true`) from short-lived browser cache with forced mount refetch, reducing seat-selection restore latency on repeat reloads while preserving live seat validation.
- `E4-005` started (startup UX slice): when active seat list is cache-seeded, mount-time `/seats?active_only=true` refetch now schedules on browser idle (timeout fallback) and disables focus-refetch churn, reducing repeat-load startup fan-out while preserving eventual seat validation.
- `E4-005` started (backend query slice): `/seats?active_only=true` now applies non-sudo buyer allow-lists directly in SQL (`buyer_id = ANY(...)`) rather than fetching all active seats then filtering in Python, reducing seat bootstrap latency/payload on large multi-seat accounts.
- `E4-005` started (backend cache slice): `SeatsService.get_buyer_seat_with_bidder` now uses short TTL caching with write-path invalidation, reducing repeated buyer->bidder lookup round trips across settings/QPS startup flows.
- `E4-005` started (backend cache slice): `SeatsService` now applies short TTL caching for seat-list reads (`get_buyer_seats`, `get_buyer_seats_by_ids`) with write/sync/link/display-name invalidation, reducing repeated `/seats` bootstrap query latency across reload cycles.
- `E4-005` started (startup UX slice): QPS Home now seeds deferred `/analytics/spend-stats` and `/analytics/home/endpoint-efficiency` payloads from short-lived browser cache so CPM badge and endpoint panel can render immediately on repeat reloads.
- `E4-005` started (startup UX slice): browser seed cache max-age is now 30 minutes (up from 15) for startup QPS datasets and deferred endpoint header cache, increasing repeat-reload immediate-render hit rate while live refetch reconciliation remains in place.
- `E4-005` started (startup UX slice): when startup CPM (`/analytics/spend-stats` with no expanded config) is cache-seeded, live refetch now schedules on browser idle (timeout fallback) instead of immediate table-settle fetch, while expanded-config CPM refresh remains immediate.
- `E4-005` started (startup UX slice): QPS Home now seeds deferred `/analytics/home/funnel` payloads from short-lived buyer+day browser cache so funnel messaging and summary values can render immediately on repeat reloads while live refetch runs.
- `E4-005` started (startup UX slice): when deferred funnel/endpoint panels are cache-seeded, their live refetch now runs on browser idle (timeout fallback) instead of immediately on table-settle, reducing post-hydration contention while preserving eventual freshness.
- `E4-005` started (startup UX slice): Account Endpoints header now seeds `/settings/endpoints` from short-lived buyer-scoped browser cache (with forced mount refetch) so deferred endpoint rows render immediately on repeat reloads while still reconciling with live API.
- `E4-005` started (startup UX slice): when deferred `/settings/endpoints` header is cache-seeded, its live refetch now also schedules on browser idle (timeout fallback) instead of immediate settle-gate fetch, reducing post-hydration contention while preserving eventual reconciliation.
- `E4-005` started (backend cache/query slice): `EndpointsService` now applies short TTL caching for endpoint-list/current-QPS reads with sync/refresh invalidation, and local `/settings/endpoints` path now fetches rows+current-QPS in parallel (`asyncio.gather`) to reduce deferred endpoint-header latency.
- `E4-005` started (backend cache slice): `AnalyticsService.get_spend_stats` now uses short TTL caching by days/buyer/billing, and `/analytics/home/refresh` now clears spend-stats cache with Home payload caches so manual refreshes stay fresh while repeated deferred CPM reads are cheaper.
- `E4-005` started (backend cache slice): startup-path service cache TTLs are now tuned from `15s` to `30s` across pretargeting, Home analytics payloads, deferred spend-stats, and endpoints services, reducing repeat-reload query churn while preserving existing write/refresh invalidation behavior.
- `E4-005` started (startup UX slice): QPS Home table-settle/delayed-state logic now treats cached config-performance seeds as hydration-ready during live refetch, preventing unnecessary delayed gating/banners when cached performance data is already usable.
- `E4-005` started (UX parity slice): QPS Home manual refresh now explicitly refetches endpoint-efficiency so operator refresh behavior remains complete after startup deferrals.
...
- `QA-004` started (API contract slice): summary API tests now also validate `bucket_hours` and `bucket_limit` forwarding for trend-bucket queries.

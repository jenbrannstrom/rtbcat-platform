...
- `E4-002` started (frontend setup slice): conversion-source readiness on `/setup` now uses both `/conversions/health` lag and `/conversions/ingestion/stats` accepted volume (14-day window) to avoid false-ready states from stale historical totals.

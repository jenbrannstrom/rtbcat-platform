...
- `E4-005` started (startup UX slice): pretargeting startup cache writes are now bounded (`max 500 rows`, `200-row` fallback on quota failure) to prevent silent `localStorage` overflow on large seat payloads.

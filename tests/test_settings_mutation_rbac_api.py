...
def _build_actions_client() -> TestClient:
    app = FastAPI()
...
def _build_changes_client() -> TestClient:
    app = FastAPI()
...
def _build_endpoints_client() -> TestClient:
    app = FastAPI()
...
def _build_pretargeting_client() -> TestClient:
    app = FastAPI()
...
def _build_snapshots_client() -> TestClient:
    app = FastAPI()
...
def _build_optimizer_client(require_admin_override) -> TestClient:
    class _StubStore:
        def __init__(self):
            self.set_calls = 0
            self.audit_calls = 0

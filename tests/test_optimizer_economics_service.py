...
async def test_get_assumed_value_returns_score(monkeypatch: pytest.MonkeyPatch):
    ...
    async def _stub_query_one_with_timeout(
        ...
        if "COALESCE(SUM(reached_queries), 0)::bigint AS reached_queries" in sql and "FROM rtb_daily" in sql:
            return {
        ...
        if "SELECT COALESCE(SUM(spend_micros), 0)::bigint AS spend_micros" in sql and "FROM rtb_daily" in sql:
            return {"spend_micros": 8_000_000}

...
@pytest.mark.asyncio
async def test_get_publisher_rows_embeds_total_count_in_same_query(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, object] = {}
    ...
    repo = HomeAnalyticsRepository()
    expected_start, expected_end = repo.get_window_bounds(7)
    rows = await repo.get_publisher_rows(days=7, buyer_id="buyer-1", limit=30)

    assert rows == []
    sql_upper = str(captured["sql"]).upper()
    assert "WITH GROUPED AS" in sql_upper
    assert "COUNT(*) OVER ()::INT AS TOTAL_PUBLISHERS" in sql_upper
    assert "LIMIT %S" in sql_upper
    assert captured["params"] == (expected_start, expected_end, "buyer-1", 30)


@pytest.mark.asyncio
async def test_get_geo_rows_embeds_total_count_in_same_query(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, object] = {}
    ...
    repo = HomeAnalyticsRepository()
    expected_start, expected_end = repo.get_window_bounds(7)
    rows = await repo.get_geo_rows(days=7, buyer_id="buyer-1", limit=30)

    assert rows == []
    sql_upper = str(captured["sql"]).upper()
    assert "WITH GROUPED AS" in sql_upper
    assert "COUNT(*) OVER ()::INT AS TOTAL_COUNTRIES" in sql_upper
    assert "LIMIT %S" in sql_upper
    assert captured["params"] == (expected_start, expected_end, "buyer-1", 30)

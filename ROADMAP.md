...
- [x] **Exception passthrough hardening (2026-03-04)** — Added explicit `except HTTPException: raise` passthrough in settings/recommendations/retention handlers, upload/import observers (`uploads`, `gmail`), seat maintenance routes (`seats/populate`), and operational ingestion routes (`collect`, `performance`, conversion replay/DLQ wrapper, `POST /performance/import-csv`) that previously relied on broad `except Exception` blocks, preserving intended status semantics while keeping failure recording (with route-level regression coverage in `tests/test_settings_exception_passthrough_api.py`, `tests/test_exception_passthrough_observer_routes_api.py`, and `tests/test_performance_exception_passthrough_api.py`).
- [x] **Router passthrough static guard (2026-03-04)** — Added AST-based regression coverage to fail CI when a top-level route `try` block remaps `Exception` to `HTTPException` without explicit `except HTTPException` passthrough in the same block (`tests/test_router_http_exception_passthrough_guard.py`).

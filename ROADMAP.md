...
- [x] **GitHub deploy workflow defaults corrected for SG production**
  - Fixed `deploy.yml` defaults to match the intended SG deployment target and registry layout.
...
- [x] **Exception passthrough hardening (2026-03-04)** — Added explicit `except HTTPException: raise` passthrough in settings/recommendations/retention handlers, upload/import observers (`uploads`, `gmail`), seat maintenance routes (`seats/populate`), and operational ingestion routes (`collect`, `performance`, conversion replay/DLQ wrapper, `POST /performance/import-csv`, settings pending-change/endpoints sync/update routes) that previously relied on broad `except Exception` blocks, preserving intended status semantics while keeping failure recording (with route-level regression coverage in `tests/test_settings_exception_passthrough_api.py`, `tests/test_exception_passthrough_observer_routes_api.py`, and `tests/test_performance_exception_passthrough_api.py`).
- [x] **Router passthrough static guard (2026-03-04)** — Added AST-based regression coverage to fail CI when a route `try` block in `api/routers/**` remaps `Exception` to `HTTPException` without explicit `except HTTPException` passthrough in the same block (`tests/test_router_http_exception_passthrough_guard.py`).
...
- [x] **Operational router type annotations (2026-03-04)** — Added explicit return-type annotations for collection/troubleshooting/system/conversions/performance/creatives/qps/config router handlers to raise static-safety/readability on active operational and telemetry endpoints without behavior changes.
- [x] **Small-router annotation sweep (2026-03-04)** — Added explicit return-type annotations on active routers (`gmail`, `precompute`, `creative_language`, `creative_geo_linguistic`, `creatives_live`, `creative_cache`, `optimizer_economics`, `optimizer_scoring`, `optimizer_workflows`, `optimizer_models`, `optimizer_proposals`, `seat_admin`, `uploads`, `docs`) to shrink remaining untyped API surface without behavior changes.
- [x] **Admin/seat + nested router annotation closure (2026-03-04)** — Added explicit return-type annotations for `admin`, `seats`, nested `analytics/*`, and `settings/optimizer` route handlers, completing explicit return annotations across `api/routers/**` route handlers.
- [x] **Router annotation regression gate (2026-03-04)** — Added AST-based test coverage to fail CI if any `@router.*` handler in `api/routers/**` is missing an explicit return annotation (`tests/test_router_handler_annotations.py`).
- [x] **QPS deprecation contract hardening (2026-03-04)** — Fixed legacy-deprecation response shape so `/qps/summary` remains schema-compatible and `/qps/include-list` no longer references removed analyzers, with service-level contract tests (`tests/test_qps_service_deprecation_contract.py`).
- [x] **Analytics service exception observability hardening (2026-03-04)** — Kept billing-id fallback behavior in `AnalyticsService` while eliminating silent masking: repository failures now emit warning logs with stack traces in bidder/billing resolution helpers, with regression coverage (`tests/test_analytics_service_exception_observability.py`).
- [x] **System service exception observability hardening (2026-03-04)** — Preserved non-blocking fallback behavior in `SystemService` while removing silent masking on node/ffmpeg/DB geo lookups: failures now emit warning logs with stack traces, with regression coverage (`tests/test_system_service_exception_observability.py`).
- [x] **Admin/actions service annotation closure (2026-03-04)** — Added explicit return-type annotations on remaining untyped `AdminService` and `ActionsService` methods (`get_user_permissions`, buyer-seat permission grant/list, permission grant, audit-log listing, and `_get_pretargeting_client`), closing the remaining service-level return annotation gaps in `services/*.py`.
- [x] **Seats router sync observability hardening (2026-03-04)** — Reduced silent masking in seat discovery/sync paths by logging fallback credential failures and preserving `HTTPException` detail in seat/bidder sync error aggregation (`sync-all`), while keeping partial-sync behavior. Background language-analysis error persistence now remains non-blocking if DB writes fail, with targeted resilience coverage (`tests/test_seats_language_analysis_resilience.py`).
- [x] **Creative performance service observability hardening (2026-03-04)** — Preserved fallback semantics in `CreativePerformanceService` (`active ids`, perf data, country map/breakdown) while adding structured debug logs with stack traces for repository failures, with regression coverage (`tests/test_creative_performance_service_exception_observability.py`).
- [x] **RTB bidstream service exception observability hardening (2026-03-04)** — Preserved fallback semantics in `RtbBidstreamService` (buyer billing lookup, creative bid totals hydration, bid-filtering hydration, language mismatch fallback) while adding structured warning/debug logs with stack traces for repository/import failures, with regression coverage (`tests/test_rtb_bidstream_service_exception_observability.py`).
- [x] **System router fallback observability hardening (2026-03-04)** — Removed silent masking in `/health` helper paths by logging service-account and DB-probe fallback failures and logging git metadata fallback failures in `get_git_sha`, with regression coverage (`tests/test_system_router_exception_observability.py`).
- [x] **Evaluation service exception observability hardening (2026-03-04)** — Preserved recommendation fallback semantics across filtered-bids/size/geo/publisher/opportunity analyzers and summary/funnel helpers while adding structured warning logs with stack traces for repository failures, with regression coverage (`tests/test_evaluation_service_exception_observability.py`).
- [x] **Data health service exception observability hardening (2026-03-04)** — Preserved fallback semantics in ingestion/report-completeness/quality-freshness/seat-day-completeness/materialized-view refresh paths while adding structured warning logs with stack traces for fallback-triggering query/refresh failures, with regression coverage (`tests/test_data_health_service_exception_observability.py`).
- [x] **Thumbnails service exception observability hardening (2026-03-04)** — Preserved thumbnail fallback semantics (ffmpeg exception classification, VAST regex fallback, retry-count coercion) while adding structured warning/debug logs with stack traces for parse/exec/cast failures, with regression coverage (`tests/test_thumbnails_service_exception_observability.py`).
- [x] **Optimizer economics service exception observability hardening (2026-03-04)** — Preserved economics fallback semantics for trend/daily/quality query timeouts while adding structured warning logs with stack traces and range context on fallback paths, with regression coverage (`tests/test_optimizer_economics_service_exception_observability.py`).
- [x] **Seats service credential fallback observability hardening (2026-03-04)** — Preserved credential resolution order while removing silent masking in legacy `ConfigManager` fallback failures (`get_credentials_with_fallback`), now warning with stack traces before continuing to ADC/validation, with regression coverage (`tests/test_seats_service_exception_observability.py`).
- [x] **Actions service parse fallback observability hardening (2026-03-04)** — Preserved `_parse_json_list` fallback semantics on invalid JSON while adding structured warning logs with stack traces/value preview, with regression coverage (`tests/test_actions_service.py`).
- [x] **Secrets manager parse fallback observability hardening (2026-03-04)** — Preserved `get_int` defaulting behavior on invalid values while adding structured warning logs with stack traces for parse failures, with regression coverage (`tests/test_secrets_manager_exception_observability.py`).
- [x] **Creative evidence VAST fallback observability hardening (2026-03-04)** — Preserved regex fallback behavior when VAST XML parsing fails while adding debug logging with stack traces for parse failures, with regression coverage (`tests/test_creative_evidence_service.py`).
- [x] **Optimizer model crypto fallback observability hardening (2026-03-04)** — Preserved crypto fallback semantics (hashed-key normalization and disabled decrypt on init failure) while adding debug/warning logs with stack traces for key decode/init failures, with regression coverage (`tests/test_optimizer_model_crypto_exception_observability.py`).
- [x] **Creative response builder env-parse fallback observability hardening (2026-03-04)** — Preserved stale-threshold default/clamp behavior while adding warning logs with stack traces for invalid `CREATIVE_CACHE_STALE_HOURS` parsing, with regression coverage (`tests/test_creative_response_builder_exception_observability.py`).
- [x] **Rule-based clustering parse fallback observability hardening (2026-03-04)** — Preserved `extract_domain`/`get_week_key` fallback semantics while adding debug logs with stack traces for URL/timestamp parse failures, with regression coverage (`tests/test_rule_based_exception_observability.py`).
- [x] **Creative destination resolver parse fallback observability hardening (2026-03-04)** — Preserved `classify_click_destination` invalid-URL fallback semantics and `_is_appsflyer_url` parse-loop fallback semantics while adding debug logs with stack traces for URL parse failures, with regression coverage (`tests/test_creative_destination_resolver_exception_observability.py`).
- [x] **Optimizer models parser fallback observability hardening (2026-03-04)** — Preserved `_to_json_obj`/`_try_json` fallback semantics while adding debug logs with stack traces on JSON-parse fallback paths, with regression coverage (`tests/test_optimizer_models_service.py`).
- [x] **Campaign router domain-parse fallback observability hardening (2026-03-04)** — Preserved `_extract_reference_domain` fallback semantics while adding debug logs with stack traces on URL-parse failures, with regression coverage (`tests/test_campaigns_router_exception_observability.py`).
- [x] **Conversion mapping-profile parse fallback observability hardening (2026-03-04)** — Preserved `_load_field_mapping_profile` invalid-JSON fallback semantics while adding warning logs with stack traces + source/scope context, with regression coverage (`tests/test_conversions_api.py`).

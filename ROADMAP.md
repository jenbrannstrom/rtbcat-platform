...
- [x] **Admin/seat router annotation closure (2026-03-04)** — Added explicit return-type annotations for `admin` and `seats` route handlers, completing top-level return annotations across `api/routers/*.py` handlers.
- [x] **Router annotation regression gate (2026-03-04)** — Added AST-based test coverage to fail CI if any `@router.*` handler in `api/routers/*.py` is missing an explicit return annotation (`tests/test_router_handler_annotations.py`).

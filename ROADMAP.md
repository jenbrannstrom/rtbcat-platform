...
- [x] **Small-router annotation sweep (2026-03-04)** — Added explicit return-type annotations on small active routers (`gmail`, `precompute`, `creative_language`, `creative_geo_linguistic`, `creatives_live`, `creative_cache`, `optimizer_economics`, `optimizer_scoring`, `optimizer_workflows`, `optimizer_models`, `optimizer_proposals`, `seat_admin`) to shrink remaining untyped API surface without behavior changes.

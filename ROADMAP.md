...
- [x] **Exception passthrough hardening (2026-03-04)** — Added explicit `except HTTPException: raise` passthrough in settings/recommendations/retention handlers, upload/import observers (`uploads`, `gmail`), and operational ingestion routes (`collect`, `performance`, conversion replay/DLQ wrapper) that previously relied on broad `except Exception` blocks, preserving intended status semantics while keeping failure recording.
